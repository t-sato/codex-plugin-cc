...
- Map `sol` to `--model gpt-6-sol` and `luna` to `--model gpt-6-luna`.
...
- If the forwarded request includes `--model`, normalize `spark`, `astra`, `sol`, and `luna` to their model IDs, then pass it through to `task`.
