...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|spark|astra|sol|luna>] [--effort <none|minimal|low|medium|high|xhigh|max|ultra>] [what Codex should investigate, solve, or continue]"
...
- Leave the model unset unless the user explicitly asks for one. Map `spark` to `gpt-5.3-codex-spark`, `astra` to `gpt-6-astra`, `sol` to `gpt-6-sol`, and `luna` to `gpt-6-luna`.
